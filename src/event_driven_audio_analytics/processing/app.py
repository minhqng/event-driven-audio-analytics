...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse the processing CLI command."""
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command == "preflight":
        logger.info("Running processing preflight.")
        try:
            check_runtime_dependencies(settings)
        except Exception:
            logger.bind(failure_class="startup_dependency").exception(
                "Processing preflight failed."
            )
            raise
        logger.info("Processing preflight passed.")
        return

    pipeline = ProcessingPipeline(settings=settings)
    logger.info(
        "Starting processing pipeline steps=%s bootstrap_servers=%s consumer_group=%s",
        pipeline.describe(),
